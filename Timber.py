...
def getFilename(configParser):
    ...
    if VERBOSE: print "Config shows filename:",myFilename
    t = Template(myFilename)
    myFilename = t.safe_substitute(stringSubstitutions)
    if VERBOSE: print "Target filename after substitutions:",myFilename
    return myFilename
...
if __name__ == '__main__':
    ...
    if re.search(r'\$\{\w*\}', FILENAME):
        if VERBOSE: print "Midnight Reload enabled."
        midnightReload = True
    else:
        midnightReload = False
    ...
    while True:
        try:
            ...
            if not line:
                ...
                #"midnight reload" -- since you can have date substitutions in the filename, first loop of new day reloads it
                if midnightReload and lastDate != datetime.now().strftime('%d'):
                    if VERBOSE: print "Date has changed, midnight reload of target file."
                    getFilename(config)
                    getWatchFile()
                    lastDate = datetime.now().strftime('%d')
